class DataSourceController < ApplicationController
  # The data source controller can be considered the refactoring process
  # We take our data from our data sources ie Links and we refactor it
  # Then save the clean data into our database
  # From there we then use our api to allow access for our other applications to use this data
  ## NOTE: Each action below is connecting to a new datasource, the "how" may vary.
  ## Most are done via connecting to an FTP and extracting the approriate csv and looping through that.
  
  # First we require a few things
  require 'time'
  require 'date'
  require 'net/ftp'
  require 'csv'
  ...
  def levels
    ...
    ftp.passive = true
    # Save to file to a tmp_path
    ftp.getbinaryfile("CodeCabin_Levels_Hist.csv", "tmp_path")
    # Open the tmp_path
    csv_text = open("tmp_path")
    csv = CSV.parse(csv_text, :headers=>true)
